...
export const useFetchCampaign = (
  ...
  if (!campaign) {
    return {
      loading,
      error: new Error(
        "Query did not return any campaign with the selected id."
      ),
    };
  }

  const entriesMetadata: EntryMetadata[] = data.entriesMetadata.map((e) => ({
    id: e.id,
  ...
  const categoryMetata: Category[] = data.categories.map((c) => ({
    id: c.id,
    campaignId: campaignId,
    title: c.title,
  ...
  return {
    data: {
      campaign,
      campaignItemsMetadata: [...entriesMetadata, ...categoryMetata],
    },
    loading,
    error,
  };
