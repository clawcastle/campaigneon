...
@app.post("/users")
async def user_registered_callback(
    request: CreateUserRequest, secret: Annotated[str, Header()]
):
    auth0_campaigneon_secret = os.environ.get("AUTH0_CAMPAIGNEON_SECRET")

    if secret != auth0_campaigneon_secret:
